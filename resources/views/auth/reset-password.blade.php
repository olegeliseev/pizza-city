    ...
    <section class="auth-section">
        <div class="container">
            <div class="auth-section__content">
                <h1 class="auth-section__title">Сброс пароля</h1>

                @if (session('status'))
                    <x-panels.messages.success :messages="(array) session('status')"/>
                @endif

                <form class="auth-section__form" action="{{ route('password.store') }}" method="POST">
                    @csrf
                    ...
                    <div class="form-footer">
                        <button class="form-btn" type="submit">Сбросить пароль</button>
                    </div>
                </form>
            </div>
        </div>
